...
export async function POST(req: Request) {
  try {
    ...
    const parsed = customEmailSchema.safeParse(json);
    ...
    const { applicationId, studentEmail, studentName, subject, message, status } = parsed.data;
    ...
    if (resend && process.env.FROM_EMAIL) {
      const customEmailHtml = `
        ...
        <head>
          ...
          <style>
            ...
            .message-content { color: #333; font-size: 14px; line-height: 1.6; margin-bottom: 16px; white-space: pre-wrap; }
      ...
      return NextResponse.json({ 
        success: true, 
        message: `Custom email sent to ${studentName}`,
    ...
    return NextResponse.json({ 
      success: true, 
      message: "Custom email processed" 
    });

  } catch (error) {
    console.error("Error processing custom email:", error);
    return NextResponse.json({ 
      error: "Failed to process custom email", 
